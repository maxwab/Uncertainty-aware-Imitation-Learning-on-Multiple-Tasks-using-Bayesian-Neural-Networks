...
def Validate_BBB_Controller(domain_name, task_identity, window_size):
    configuration_identity = domain_name + '_' + str(task_identity) + '_' + str(window_size) + '_BBB'
    ...
    file_to_save_logs = LOGS_DIRECTORY + configuration_identity + '.pkl'
    ...
    for task_to_validate in ALL_MUJOCO_TASK_IDENTITIES:
        ...
        for validation_trial in range(NUMBER_VALIDATION_TRIALS):
            ...
            env = get_contextual_MUJOCO_environment(env_name=domain_name, context=str(task_to_validate))
...
if __name__ == '__main__':
    ...
    Validate_BBB_Controller(domain_name=args.domain_name, task_identity=args.task_identity, window_size=args.window_size)