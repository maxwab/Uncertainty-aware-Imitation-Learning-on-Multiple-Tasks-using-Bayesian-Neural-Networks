...
def train_BBB(data_x, data_y, configuration_identity, epochs, number_mini_batches, activation_unit, learning_rate, hidden_units, number_samples_variance_reduction, precision_alpha, weights_prior_mean_1, weights_prior_mean_2, weights_prior_deviation_1, weights_prior_deviation_2, mixture_pie, rho_mean, extra_likelihood_emphasis):
    directory_to_save_tensorboard_data = configuration_identity + 'training/' + TENSORBOARD_DIRECTORY
    saved_models_during_iterations_bbb = configuration_identity + 'training/' + SAVED_MODELS_DURING_ITERATIONS_DIRECTORY
    saved_final_model_bbb = configuration_identity + 'training/' + SAVED_FINAL_MODEL_DIRECTORY
...
def validate_BBB(domain_name, task_identity, configuration_identity):
    #tf.reset_default_graph()
    copycat_graph = tf.Graph()
    with copycat_graph.as_default():
        copycat_controller = Load_BBB(configuration_identity=configuration_identity)

    file_to_save_logs = configuration_identity + 'validation_logs.pkl'

    logs_for_all_tasks = {}
    for task_to_validate in tqdm(ALL_MUJOCO_TASK_IDENTITIES):
        logs_for_a_task = {}
...
def run_on_itself(domain_name, task_identity, configuration_identity, detector=None):
    #tf.reset_default_graph()
    copycat_graph = tf.Graph()
    with copycat_graph.as_default():
        copycat_controller = Load_BBB(configuration_identity=configuration_identity)
    ...
    while not finish:
        all_behavior_deviation.append(behavior_var_control)
        if not detector is None:
            isSafe, stats = detector.isSafeToContinue(behavior_var_control)
            if not str_to_bool(isSafe):
                return 'False', np.mean(all_behavior_deviation)
    ...
    return 'True', np.mean(all_behavior_deviation)
...
def data_efficient_imitation_across_multiple_tasks(domain_name, window_size, number_demonstrations, adapt_detector_threshold, start_monitoring_at_time_step, detector_c, detector_m, initial_detector_threshold, epochs, number_mini_batches, activation_unit, learning_rate, hidden_units, number_samples_variance_reduction, precision_alpha, weights_prior_mean_1, weights_prior_mean_2, weights_prior_deviation_1, weights_prior_deviation_2, mixture_pie, rho_mean, extra_likelihood_emphasis):
    for simulation_iterator in range(TOTAL_SIMULATION_ITERATIONS):
        ...
        for task_iterator, current_task_identity in enumerate(ALL_MUJOCO_TASK_IDENTITIES):
            ...
            meta_data_file_for_this_run = 'logs/' + domain_name + '/naive_controller/' + str(simulation_iterator) + '/meta_data.pkl'
            meta_data_for_this_run = {LAST_TRAINING_TASK_ITERATION_KEY: task_iterator}
        ...
        all_gathered_x, all_gathered_y = None, None
        tasks_trained_on, tasks_encountered = [], []
        current_task_identity = ALL_MUJOCO_TASK_IDENTITIES[0]
        ...
        for task_iterator in ALL_MUJOCO_TASK_IDENTITIES:
            ...
            configuration_identity = 'logs/' + domain_name + '/bbb_controller/' + str(simulation_iterator) + '/' + str(task_iterator) + '/'
            training_logs_directory = configuration_identity + 'training/'
            if not os.path.exists(training_logs_directory):
                os.makedirs(training_logs_directory)
            ...
            if not did_succeed:
                ...
                meta_data_file_for_this_run = 'logs/' + domain_name + '/bbb_controller/' + str(simulation_iterator) + '/meta_data.pkl'
                meta_data_for_this_run = {LAST_TRAINING_TASK_ITERATION_KEY: task_iterator}
                with open(meta_data_file_for_this_run, 'wb') as f:
                    pickle.dump(meta_data_for_this_run, f)
                #need_training = False
            ...
            if task_iterator == (len(ALL_MUJOCO_TASK_IDENTITIES) - 1):
                break

            current_task_identity = ALL_MUJOCO_TASK_IDENTITIES[task_iterator + 1]
            tasks_encountered.append(current_task_identity)
