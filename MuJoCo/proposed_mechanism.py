...
def run_on_itself(domain_name, task_identity, controller_identity, detector=None):
    #tf.reset_default_graph()
    copycat_graph = tf.Graph()
    with copycat_graph.as_default():
        copycat_controller = Load_BBB(controller_identity=controller_identity)
    ...
    while not finish:
        all_behavior_deviation.append(np.sqrt(behavior_var_control))
        if not detector is None:
            isSafe, stats = detector.isSafeToContinue(np.sqrt(behavior_var_control))
        ...
        if controller == 'BBB':
            ...
            for task_iterator in range(len(COPY_OF_ALL_MUJOCO_TASK_IDENTITIES)):
                ...
                if not did_succeed:
                    ...
                    _, average_uncertainty, _ = run_on_itself(domain_name=domain_name, task_identity=current_task_identity, controller_identity= current_controllers_identity)
                ...
                tasks_encountered.append(current_task_identity)
                did_succeed, average_uncertainty, stats = run_on_itself(domain_name=domain_name, task_identity=current_task_identity, controller_identity=current_controllers_identity, detector=detector)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-nd', '--number_demonstrations', type=int, help='Number demonstrations per request', default=50)
    ...
    data_efficient_imitation_across_multiple_tasks(controller=args.controller_type, domain_name=args.domain_name, window_size=args.window_size, number_demonstrations=args.number_demonstrations, adapt_detector_threshold=str_to_bool(args.adapt_detector_threshold),
