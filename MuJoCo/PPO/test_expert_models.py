...
def getScaleAndOffset(env_name, expert_task_identity):
	FILE_NAME = './../' + EXPERT_TRAJECTORIES_DIRECTORY + env_name + '_' + expert_task_identity + '.pkl'
...
def load_trained_model(env_name): 
	...
	for expert_task_identity in ALL_MUJOCO_TASK_IDENTITIES:
		expert_task_identity = str(expert_task_identity)
		scale, offset = getScaleAndOffset(env_name, expert_task_identity)

		saved_final_model = './../' + SAVED_EXPERT_MODELS_DIRECTORY + env_name + '/' + expert_task_identity + '/'
		...
		with tf.Session() as sess:  
			...
			for validation_task_identity in ALL_MUJOCO_TASK_IDENTITIES:
				...
				if expert_task_identity == validation_task_identity:
			...
			plotExperts(env_name=env_name, expert_rewards_over_all_contexts=expert_rewards_over_all_contexts, identifier=str(expert_task_identity))
