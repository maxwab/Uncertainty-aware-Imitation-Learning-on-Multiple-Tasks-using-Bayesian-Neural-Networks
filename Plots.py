...
def final_flourish(all_task_configurations, uncertainty_ylim, cost_ylim, predictive_error_ylim):
    ...
    for configuration in all_task_configurations:
        ...
        if configuration[BEHAVIORAL_CONTROLLER_KEY] == 'GP':          
            plt.plot(all_tasks, average_predictive_error, label=configuration_label, color=matplotlibcolors[iterator], ls = '--', lw=0.8)
            iterator = iterator + 1
        else:
            plt.plot(all_tasks, average_predictive_error, label=configuration_label, color=matplotlibcolors[iterator], lw=0.8)  
        
    plt.xlabel('Block Mass', fontweight='bold')
    plt.xticks(np.linspace(1., 100., 10))
    plt.ylabel('Predictive Mean Squared Error', fontweight='bold')
